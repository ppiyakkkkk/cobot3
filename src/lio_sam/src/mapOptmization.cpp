...
public:
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<lio_sam::srv::SaveMap::Request> req, std::shared_ptr<lio_sam::srv::SaveMap::Response> res) -> void {
            ...
            {
                RCLCPP_WARN(get_logger(), "No keyframes yet, skip saving (empty map).");
                res->success = false;
                return;
            }
    ...
    void publishOdometry()
    {
        ...
        // Publish TF
        // lidarFrame==baselinkFrame이면 TransformFusion(imuPreintegration.cpp)이 이미
        // 같은 odom->lidar_link를 방송하고 있어서, 여기서 또 방송하면 같은 자식 프레임에
        // 서로 다른 값(10Hz 맵최적화 vs 100Hz IMU보간)이 겹쳐써져 TF가 튄다. 이 lio_sam
        // 코드베이스 안에서 이 TF를 lookupTransform으로 구독하는 곳이 없으므로 안전하게 건너뛴다.
