...
public:
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        // publish tf
        // frame_id/stamp 부여는 lidarFrame==baselinkFrame일 때도 반드시 필요하다
        // (안 그러면 tCur에 frame_id가 안 실려서 tf2가 아래 sendTransform을
        // "frame_id not set"으로 통째로 무시한다). lidar2Baselink 조회(lookupTransform)만
        // 실제로 프레임이 다를 때에 한해서 수행한다.
        tf2::Transform tCurTransform = tCur;
        if(lidarFrame != baselinkFrame)
        {
            ...
            tCurTransform = tCurTransform * lidar2Baselink;
        }
        tf2::Stamped<tf2::Transform> tb(
            tCurTransform, tf2_ros::fromMsg(odomMsg->header.stamp), odometryFrame);
        tCur = tb;
