...
class ObstacleMonitorNode(TimestampedNode):
    def __init__(self):
        ...
        # 조밀한 숲에서는 거의 측면인 통로도 후보로 사용하되, 90°를
        # 넘는 후방 방향은 허용하지 않는다.
        self.declare_parameter("candidate_max_offset_deg", 80.0)
        # A*=NONE이어도 FGM(Follow the Gap Method)으로 LiDAR의 연속된
        # 빈 구간(gap)에서 진행 방향을 만든다.
        # 좁은 숲길을 고정 각도 후보가 놓치지 않도록 5도 간격으로 검사한다.
        ...
        self.declare_parameter("gap_side_switch_penalty", 5.0)
    ...
    def _select_gap_following_direction(
        self,
        angles,
        distances,
        target_center,
        ...
        """목표 전방 ±80°에서 연속된 빈 LiDAR 구간을 찾아 방향을 고른다.

        완전한 격자 경로가 없어도 실제 센서에 열린 통로가 있으면 방향을
        제공한다. 선택한 좌/우는 잠시 유지하지만, 모든 후보는 목표 기준
        ±80° 안이라 후진 성분을 만들지 않는다.
        """
        if not bool(self.get_parameter("gap_following_enabled").value):
            return 0.0, 0.0, False

        max_offset = min(
            89.0,
            abs(float(self.get_parameter("candidate_max_offset_deg").value)),
        )
        step = max(
            2.0,
            float(self.get_parameter("gap_sample_step_deg").value),
        )
        half_angle = math.radians(
            max(
                step * 0.5,
        ...
        minimum_clearance = max(
            0.1,
            float(self.get_parameter("gap_min_clearance_m").value),
        )
        minimum_width = max(
            step,
            float(self.get_parameter("gap_min_width_deg").value),
        )
        width_weight = float(
            self.get_parameter("gap_width_weight").value
        )
        forward_weight = float(
            self.get_parameter("gap_forward_weight").value
        )
        switch_penalty = float(
            self.get_parameter("gap_side_switch_penalty").value
        )
        ...
        for offset_deg in offsets:
            candidate_angle = self._wrap_angle(
                target_center + math.radians(float(offset_deg))
            )
            mask = (
                np.abs(self._angle_difference(angles, candidate_angle))
                <= half_angle
            ...
            if math.isnan(clearance):
                clearance = 0.0
            elif math.isinf(clearance):
                clearance = float(
                    self.get_parameter(
                        "candidate_score_distance_cap_m"
                    ).value
                )
        ...
        gaps = []
        start = None
        for index, clearance in enumerate(clearances + [0.0]):
            if clearance >= minimum_clearance and start is None:
                start = index
            elif clearance < minimum_clearance and start is not None:
                end = index - 1
                width_deg = (end - start + 1) * step
                if width_deg >= minimum_width:
                    gaps.append((start, end, width_deg))
                start = None

        now = float(measurement_time)
        if now < self.last_avoidance_selection_time:
            self.preferred_avoidance_side = 0
            self.preferred_avoidance_side_until = float("-inf")
        self.last_avoidance_selection_time = now
        ...
        candidates = []
        for start, end, width_deg in gaps:
            # 통로의 가장 먼 한 점보다 중앙에 가까운 안전한 방향을 택한다.
            best_index = max(
                range(start, end + 1),
                key=lambda idx: (
                    min(idx - start, end - idx),
                    clearances[idx],
                    -abs(offsets[idx]),
                ),
            )
            offset_deg = float(offsets[best_index])
            side = 1 if offset_deg > step * 0.5 else (
                -1 if offset_deg < -step * 0.5 else 0
            )
            side_cost = (
                switch_penalty
                if (
                    side_hold_active
                    and side != 0
                    and side != self.preferred_avoidance_side
                )
                else 0.0
            )
            forward_ratio = math.cos(math.radians(offset_deg))
            score = (
                float(clearances[best_index])
                + width_weight * width_deg
                + forward_weight * forward_ratio
                - side_cost
            )
            candidates.append(
                (score, offset_deg, clearances[best_index], side)
            )

        if not candidates:
            return 0.0, 0.0, False

        best = max(candidates, key=lambda item: (item[0], -abs(item[1])))
        selected_side = int(best[3])
        if selected_side != 0:
            hold_sec = max(
                0.0,
        ...
        selected_angle = self._wrap_angle(
            target_center + math.radians(float(best[1]))
        )
        return selected_angle, float(best[2]), True
