...
"""통합/PC A/PC B launch가 공유하는 산림 구조 노드 구성."""
...
ROLE_INTEGRATED = "integrated"
ROLE_PC_A = "pc_a"
ROLE_PC_B = "pc_b"
SUPPORTED_ROLES = (ROLE_INTEGRATED, ROLE_PC_A, ROLE_PC_B)
...
def _launch_nodes(context, role):
    if role not in SUPPORTED_ROLES:
        raise RuntimeError(f"지원하지 않는 실행 역할입니다: {role}")

    runs_pc_a = role in (ROLE_INTEGRATED, ROLE_PC_A)
    runs_pc_b = role in (ROLE_INTEGRATED, ROLE_PC_B)
    ...
    for index, drone_id in enumerate(drone_ids, start=1):
        ...
        if runs_pc_a:
            # PC A: PX4 제어, 센서 TF, LiDAR 로컬맵과 장애물 회피.
            nodes.extend(
                [
                    Node(
                        **common,
        ...
        # PC B: GPU 중심의 YOLO와 RGB-D 위치 추정.
        if runs_pc_b and operation_mode == "rescue_search":
            nodes.extend(
                [
                    Node(
                        **common,
...
def make_launch_description(role):
    if role not in SUPPORTED_ROLES:
        raise RuntimeError(f"지원하지 않는 실행 역할입니다: {role}")
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "use_rviz",
                default_value="false",
                description="통합 또는 PC A에서 RViz 화면을 함께 실행",
            ),
            DeclareLaunchArgument(
                "use_sim_time",
                default_value="true",
