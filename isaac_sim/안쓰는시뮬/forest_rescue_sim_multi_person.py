...
# 드론 기준 사람 생성 위치 (그림의 1, 2, 3 위치)
PERSON_RANDOM_SPAWN_POSITIONS = [
    [10.0, 0.0, 0.0],
    [-5.0, 8.0, 0.0],
    [-5.0, -8.0, 0.0],
]
...
class ForestRescueSimulation:
    def __init__(self):
        ...
        print(
            "[OK] Forest-rescue multi-person simulation initialized"
        )
        print(f"[INFO] Spawned {DRONE_COUNT} drones")
        print(f"[INFO] Spawned {len(self.people)} people")
        print("[INFO] RGB topic: /quadrotor_0/Camera/rgb")
        print("[INFO] Depth topic: /quadrotor_0/Camera/depth")
        print("[INFO] CameraInfo topic: /quadrotor_0/Camera/camera_info")
    ...
    def _spawn_people(self):
        ...
        # 사람 1명: 드론 기준 3개 위치 중 랜덤
        selected_pos_idx = self.rng.integers(0, len(PERSON_RANDOM_SPAWN_POSITIONS))
        victim_world_pos = np.array(DRONE_POSITION[:2]) + np.array(PERSON_RANDOM_SPAWN_POSITIONS[selected_pos_idx][:2])
        
        terrain_z = self.terrain.height(victim_world_pos[0], victim_world_pos[1])
        position = np.array([victim_world_pos[0], victim_world_pos[1], terrain_z + PERSON_GROUND_OFFSET_M])
        ...
        print(
            f"[INFO] Spawned victim at position {selected_pos_idx + 1} (world): "
            f"({position[0]:.3f}, {position[1]:.3f}, {position[2]:.3f})"
        )

        # 구조자 1명: 드론 근처 산 표면
        rescuer_world_x = DRONE_POSITION[0] + self.rng.uniform(-3, 3)
        rescuer_world_y = DRONE_POSITION[1] + self.rng.uniform(-3, 3)
        rescuer_terrain_z = self.terrain.height(rescuer_world_x, rescuer_world_y)
        rescuer_position = np.array([rescuer_world_x, rescuer_world_y, rescuer_terrain_z + PERSON_GROUND_OFFSET_M])
    ...
    def _spawn_iris(self):
        ...
        for drone_idx in range(DRONE_COUNT):
            ...
            # 나란히 배치
            drone_pos = [
                DRONE_POSITION[0] + drone_idx * DRONE_SPACING,
                DRONE_POSITION[1],
                DRONE_POSITION[2],
            ]
            ...
            print(f"[INFO] Spawned drone {drone_idx} at {drone_pos}")
        
        # 호환성을 위해 첫 번째 드론을 self.drone으로도 설정
        self.drone = self.drones[0]

    def _configure_drone_camera(self):
        """카메라 시야각과 하향 장착각을 Stage에 영구 적용한다. 카메라 위치를 드론 로컬 좌표 [0,0,0]으로 설정한다."""
        stage = omni.usd.get_context().get_stage()
