...
# 구조자·드론·조난자를 이 산악 환경 Prim의 자식으로 생성한다.
# 아래 배치 좌표는 모두 이 Prim을 부모로 하는 로컬 좌표다.
ENVIRONMENT_PRIM_PATH = (
    "/World/layout/korean_mountain_100x100_dense_grouped"
)

# ---------------------------------------------------------------------------
# 시작 발판 배치
# ---------------------------------------------------------------------------
# 산악 환경 Prim 기준 발판의 로컬 좌표다.
# 구조자와 드론 3대는 충돌하지 않도록 주변에 배치한다.
...
# 조난자 랜덤 스폰 위치
# ---------------------------------------------------------------------------
# 좌표는 ENVIRONMENT_PRIM_PATH 기준 로컬 좌표를 그대로 사용한다.
VICTIM_SPAWN_POINTS = (
    {
        "name": "victim_spawn_1",
...
class ForestRescueSimulation:
    def __init__(self):
        ...
        self._validate_actor_parent_prim()
        ...
        print(f"[INFO] Rescuer local position: {RESCUER_POSITION}")
        print(
            f"[INFO] Victim spawn: {self.selected_victim_spawn['name']} "
            f"at local {self.selected_victim_spawn['position']}"
        )
        for spec in DRONE_SPECS:
            name = spec["name"]
            print(
                f"[INFO] {name} local position: {spec['position']} "
    ...
    def _validate_actor_parent_prim(self):
        """구조 객체가 들어갈 실제 산악 환경 Prim 경로를 확인한다."""
        stage = omni.usd.get_context().get_stage()
        environment_prim = stage.GetPrimAtPath(ENVIRONMENT_PRIM_PATH)

        if not environment_prim or not environment_prim.IsValid():
            raise RuntimeError(
                "구조 객체의 부모가 될 산악 환경 Prim을 찾을 수 없습니다.\n"
                f"Expected Prim: {ENVIRONMENT_PRIM_PATH}"
            )

        print(
            "[CHECK] Rescue actors parent Prim: "
            f"{ENVIRONMENT_PRIM_PATH}"
        )

    @staticmethod
    def _set_prim_local_pose(prim_path, local_position, local_yaw):
        """USD Prim의 부모 기준 로컬 위치와 Yaw를 직접 설정한다."""
        stage = omni.usd.get_context().get_stage()
        prim = stage.GetPrimAtPath(prim_path)

        if not prim or not prim.IsValid():
            raise RuntimeError(
                f"로컬 자세를 설정할 Prim을 찾을 수 없습니다: {prim_path}"
            )

        xformable = UsdGeom.Xformable(prim)
        current_local_matrix = xformable.GetLocalTransformation()
        current_transform = Gf.Transform(current_local_matrix)
        current_scale = current_transform.GetScale()

        configured_transform = Gf.Transform()
        configured_transform.SetTranslation(
            Gf.Vec3d(
        ...
        configured_transform.SetRotation(
            Gf.Rotation(
                Gf.Vec3d(0.0, 0.0, 1.0),
                math.degrees(float(local_yaw)),
            )
        )
        configured_transform.SetScale(current_scale)

        # 기존 World 보정용 transform op를 제거하고 부모 기준 Local Transform으로 교체한다.
        xformable.ClearXformOpOrder()
        transform_op = xformable.AddTransformOp(
            UsdGeom.XformOp.PrecisionDouble
        )
        transform_op.Set(configured_transform.GetMatrix())

    def _move_person_under_environment(
        self,
        person,
        local_position,
        local_yaw,
    ):
        """Pegasus Person을 산악 환경 Prim 아래로 옮겨 로컬 좌표를 적용한다."""
        stage = omni.usd.get_context().get_stage()
        old_path = str(person._stage_prefix)
        person_name = old_path.rsplit("/", 1)[-1]
        new_path = f"{ENVIRONMENT_PRIM_PATH}/{person_name}"

        if stage.GetPrimAtPath(new_path).IsValid():
            raise RuntimeError(
                f"이동할 Person Prim 경로가 이미 존재합니다: {new_path}"
            )
        ...
        if not moved_prim or not moved_prim.IsValid():
            raise RuntimeError(
                "Person Prim을 산악 환경 Prim 아래로 이동하지 못했습니다.\n"
                f"From: {old_path}\nTo: {new_path}"
            )

        # Person 내부에서 사용하는 Prim 경로와 PeopleManager 등록 키도 갱신한다.
        people_manager = PeopleManager.get_people_manager()
        people_manager.remove_person(old_path)

        person._stage_prefix = new_path
        (
            person.character_skel_root,
            person.character_skel_root_stage_path,
        ) = Person._transverse_prim(stage, new_path)

        people_manager.add_person(new_path, person)

        self._set_prim_local_pose(
            new_path,
            local_position,
            local_yaw,
        )
        ...
        # Person 이동 로직은 World 좌표를 사용하므로 초기 목표를 실제 World 위치로 맞춘다.
        world_matrix = omni.usd.get_world_transform_matrix(
            stage.GetPrimAtPath(new_path)
        )
        ...
        person._state.position = world_position.copy()
        person._previous_position = world_position.copy()
        person._target_position = world_position.copy()
        person._target_speed = 0.0

        print(
            f"[INFO] Person parented: {new_path}, "
            f"local={list(local_position)}, "
            f"world={world_position.tolist()}"
        )
    ...
    def _spawn_rescuer(self):
        ...
        self._move_person_under_environment(
            self.rescuer,
            RESCUER_POSITION,
            local_yaw=0.0,
        )
        ...
        print(f"[INFO] Rescuer local spawn: {RESCUER_POSITION}")
    ...
    def _spawn_random_victim(self):
        ...
        print(
            f"[INFO] Victim local spawn position: {victim_position}"
        )
    ...
    def _spawn_rescue_drones(self):
        ...
        for spec in DRONE_SPECS:
            ...
            drone_prim_path = (
                f"{ENVIRONMENT_PRIM_PATH}/{spec['name']}"
            )
            drone = Multirotor(
                drone_prim_path,
                ROBOTS["Iris"],
            ...
            # Robot 생성자의 position은 World pose이므로 생성 직후 부모 기준
            # Local pose로 다시 지정한다. Quaternion은 scalar-first(w, x, y, z)다.
            drone.set_local_pose(
            ...
            print(
                f"[INFO] Drone parented: {drone_prim_path}, "
                f"local={spec['position']}"
            )
