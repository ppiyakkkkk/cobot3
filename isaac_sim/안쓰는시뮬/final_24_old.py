...
- 착륙 시험 시 지정된 XYZ 위치에 조난자를 고정 배치하며 지면 보정 모드를 지원한다.
"""
...
FOR_TEST_VICTIM_WORLD_XYZ = (3.5, 40.0, 20.0)
# True이면 X·Y만 그대로 사용하고 Z는 실제 Terrain 표면으로 자동 보정한다.
FOR_TEST_VICTIM_KEEP_ON_GROUND = True # 사람이 경사면 위에서 뜨거나 묻히지 않게 하는 기본 시험 모드다.
