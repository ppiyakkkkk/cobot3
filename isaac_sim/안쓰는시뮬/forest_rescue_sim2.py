...
# 시작 지점 배치
# ---------------------------------------------------------------------------
# 사용자가 지정한 구조팀 시작 지점이다.
START_POINT = np.array([-36.0, -37.0, 31.0], dtype=float)

# 세 객체를 정확히 같은 좌표에 놓으면 서로 겹쳐 물리 충돌이 발생한다.
# 구조자는 시작점 중앙에 두고, 드론은 X축으로 좌우 2m씩 분리한다.
#
# 주의:
# 경사 지형에서는 X/Y가 달라지면 실제 지면 Z도 달라질 수 있다.
# 아래 세 위치의 Z값은 각 스폰 지점의 지면 높이에 맞춰 개별 조정한다.
RESCUER_POSITION = (START_POINT + np.array([0.0, 0.0, 0.0])).tolist()
DRONE_1_POSITION = (START_POINT + np.array([-2.0, 0.0, 0.25])).tolist()
DRONE_2_POSITION = (START_POINT + np.array([2.0, 0.0, 0.25])).tolist()
...
# 조난자 랜덤 스폰 구역은 좌표가 정해진 뒤 추가한다.
# 예: [{"name": "zone_a", "x": (-20, -10), "y": (5, 15)}, ...]
VICTIM_SPAWN_ZONES = []
...
class NamespacedLidar(Lidar):
    ...
    def start(self):
        ...
        # 구조 드론 2대의 토픽 충돌을 피하기 위해 이름을 분리한다.
...
class ForestRescueSimulation:
    def __init__(self):
        ...
        print("[OK] Forest-rescue team initialized")
    ...
    def _build_multirotor_config(self, spec):
        ...
        # 차량 이름을 ROS 2 namespace로 사용하므로 두 드론의
        # 카메라 토픽이 자동으로 분리된다.
    ...
    def _spawn_rescue_drones(self):
        """서로 다른 PX4 vehicle_id를 가진 Iris 드론 2대를 생성한다."""
        self.drones = []
        ...
        self.drone_1, self.drone_2 = self.drones
