...
# ---------------------------------------------------------------------------
# 산악 지형의 로컬 좌표 → World 좌표 변환
# ---------------------------------------------------------------------------
# /World/layout/korean_mountain_100x100_dense_grouped의 Translate 값이다.
TERRAIN_WORLD_OFFSET = np.array([-34.0, 40.0, 30.0], dtype=float)

# ---------------------------------------------------------------------------
# 시작 발판 배치
# ---------------------------------------------------------------------------
# 산악 지형 Prim 내부에서 확인한 시작 발판의 로컬 좌표다.
START_POINT_LOCAL = np.array([-36.0, -37.0, 31.0], dtype=float)
START_POINT = TERRAIN_WORLD_OFFSET + START_POINT_LOCAL
...
# 조난자 랜덤 스폰 위치
# ---------------------------------------------------------------------------
# 아래 값은 산악 지형 Prim 내부의 로컬 좌표이며,
# Person은 /World 아래에 생성되므로 Terrain Translate 값을 더해 사용한다.
VICTIM_SPAWN_POINTS = (
    {
        "name": "victim_spawn_1",
        "position": (
            TERRAIN_WORLD_OFFSET + np.array([33.0, 29.0, 13.7])
        ).tolist(),
    },
    {
        "name": "victim_spawn_2",
        "position": (
            TERRAIN_WORLD_OFFSET + np.array([33.0, -22.0, 50.6])
        ).tolist(),
    },
    {
        "name": "victim_spawn_3",
        "position": (
            TERRAIN_WORLD_OFFSET + np.array([-0.9, -1.8, -0.9])
        ).tolist(),
    },
)
