...
DRONE_CONFIGS = [
    ("/World/quadrotor", 0, [-34.0, 40.0, 31.0]),
...
# 구조자는 드론 중심에서 Y 방향으로 1.5 m 떨어진 산 표면에 생성한다.
...
class ForestRescueSimulation:
    def __init__(self):
        ...
        print("[INFO] LiDAR topic (quadrotor only): /point_cloud")
    ...
    def _spawn_people(self):
        ...
        victim_position = VICTIM_SPAWN_POSITIONS[victim_index]
        ...
        rescuer_position = [
            rescuer_x,
            rescuer_y,
            self.terrain.height(rescuer_x, rescuer_y)
            + PERSON_GROUND_OFFSET_M,
        ]
        self.rescuer = Person(
            "rescuer_01",
            selected_asset,
