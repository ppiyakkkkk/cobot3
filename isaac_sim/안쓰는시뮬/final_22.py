...
- 착륙 시험 시 1번 드론 첫 수색 줄 중앙의 가장 잘 보이는 빈 공간에 조난자를 자동 배치한다.
"""
...
# 착륙 복귀 시험용 조난자 위치 설정이다.
# True이면 quadrotor_01의 첫 번째 수색 줄을 기준으로 위치를 자동 계산한다.
FOR_TEST_VICTIM_SPAWN_ENABLED = True
FOR_TEST_VICTIM_DRONE_ID = "quadrotor_01"

# 첫 줄의 양 끝은 제외하고 가운데 구간에서 빈 공간을 자동 탐색한다.
# 0.30~0.70이면 첫 줄 중앙 40% 구간 안에서만 조난자가 생성된다.
FOR_TEST_VICTIM_PROGRESS_MIN = 0.30
FOR_TEST_VICTIM_PROGRESS_MAX = 0.70
FOR_TEST_VICTIM_CANDIDATE_COUNT = 81

# 빈 공간 점수가 비슷하면 첫 줄 정중앙에 가까운 후보를 우선한다.
FOR_TEST_VICTIM_CENTER_PENALTY_M = 2.0

# 사람 주위에 확보되기를 원하는 최소 수평 여유 공간이다.
# 실제 환경상 이 값을 만족하지 못해도 가장 넓은 후보를 선택하고 경고한다.
FOR_TEST_VICTIM_PREFERRED_CLEARANCE_M = 2.5
...
class EnvironmentMeshExporter:
    ...
    def write(self, output_path):
        ...
        # 테스트 조난자가 나무·관목·바위에 가려지지 않는 빈 공간을
        # 찾을 때 사용할 실제 World XY 표면 점이다.
        visibility_xy_parts = []
        ...
        for prim in self._stage.Traverse():
            ...
            # 모든 정점을 그대로 KDTree에 넣으면 큰 숲에서 메모리가
            # 과도하게 늘 수 있으므로 Mesh별 최대 약 20,000점을 균일 추출한다.
            visibility_stride = max(
                1,
                int(math.ceil(len(vertices) / 20_000)),
            )
            visibility_xy_parts.append(
                vertices[::visibility_stride, :2].astype(
                    np.float64,
                    copy=False,
                )
            )
        ...
        if visibility_xy_parts:
            visibility_xy = np.concatenate(
                visibility_xy_parts,
                axis=0,
            )
            # 거의 같은 XY 정점을 줄여 KDTree 생성 비용을 낮춘다.
            visibility_xy = np.unique(
                np.round(visibility_xy, decimals=2),
                axis=0,
            )
        else:
            visibility_xy = np.empty((0, 2), dtype=np.float64)

        print(
            "[INFO] 시험 조난자 빈 공간 탐색점: "
            f"{len(visibility_xy)}개"
        )
        return visibility_xy
...
class ForestRescueSimulation:
    def __init__(self):
        ...
        environment_visibility_xy = EnvironmentMeshExporter(
            stage
        ).write(
            GENERATED_ENVIRONMENT_MESH_PATH
        )
        if len(environment_visibility_xy) > 0:
            self._environment_visibility_tree = cKDTree(
                environment_visibility_xy
            )
        else:
            self._environment_visibility_tree = None
            carb.log_warn(
                "환경 Mesh XY 점이 없어 시험 조난자 위치를 중앙값으로 "
                "대체합니다."
            )
    ...
    def _write_generated_search_plan(self):
        ...
        for config, zone_index in zip(DRONE_CONFIGS, zone_indices):
            ...
            # 1번 드론 첫 수색 줄 중앙 구간에서 나무·관목·바위
            # 표면점과 가장 멀리 떨어진 위치를 시험 조난자 위치로 선택한다.
            if (
                FOR_TEST_VICTIM_SPAWN_ENABLED
                and drone_name == FOR_TEST_VICTIM_DRONE_ID
            ):
                progress_min = float(np.clip(
                    FOR_TEST_VICTIM_PROGRESS_MIN,
                    0.0,
                    1.0,
                ))
                progress_max = float(np.clip(
                    FOR_TEST_VICTIM_PROGRESS_MAX,
                    progress_min,
                    1.0,
                ))
                candidate_count = max(
                    3,
                    int(FOR_TEST_VICTIM_CANDIDATE_COUNT),
                )
                candidate_progress = np.linspace(
                    progress_min,
                    progress_max,
                    candidate_count,
                )
                candidate_x = (
                    x_low
                    + (x_high - x_low) * candidate_progress
                )
                candidate_y = np.full_like(
                    candidate_x,
                    float(rows[0]),
                )
                ...
                if self._environment_visibility_tree is not None:
                    clearances, _ = (
                        self._environment_visibility_tree.query(
                            candidate_xy,
                            k=1,
                        )
                    )
                ...
                # 같은 정도로 빈 장소라면 첫 줄 정중앙에 가까운 곳을
                # 선택해 드론이 지나치게 늦거나 빠르게 찾지 않게 한다.
                center_penalty = (
                    np.abs(candidate_progress - 0.5)
                    * float(FOR_TEST_VICTIM_CENTER_PENALTY_M)
                )
                visibility_score = clearances - center_penalty
                best_index = int(np.argmax(visibility_score))

                progress = float(candidate_progress[best_index])
                test_x = float(candidate_x[best_index])
                test_y = float(candidate_y[best_index])
                test_clearance = float(clearances[best_index])
                ...
                plan["test_victim_spawn"] = {
                    "enabled": True,
                    "selection": "first_row_visible_clearing",
                    "drone_id": drone_name,
                    "row_index": 0,
                    "row_progress": progress,
                    "nearest_environment_clearance_m": (
                        test_clearance
                    ),
                    "world_enu": list(
                        self.test_victim_spawn_world_enu
                ...
                if (
                    test_clearance
                    < FOR_TEST_VICTIM_PREFERRED_CLEARANCE_M
                ):
                    carb.log_warn(
                        "시험 조난자 주변 여유 공간이 권장값보다 작습니다: "
                        f"actual={test_clearance:.2f}m, "
                        f"preferred="
                        f"{FOR_TEST_VICTIM_PREFERRED_CLEARANCE_M:.2f}m"
                    )

                print(
                    "[TEST] 보기 쉬운 조난자 시험 위치 선택: "
                    f"drone={drone_name}, "
                    f"first_row_progress={progress:.3f}, "
                    f"environment_clearance={test_clearance:.2f}m, "
                    f"world=({test_x:.2f}, {test_y:.2f}, "
                    f"{self.test_victim_spawn_world_enu[2]:.2f})"
                )
    ...
    def _spawn_people(self):
        ...
        if FOR_TEST_VICTIM_SPAWN_ENABLED:
            ...
            if victim_position is None:
                raise RuntimeError(
                    "시험용 조난자 위치가 수색 계획에서 생성되지 않았습니다. "
                    "FOR_TEST_VICTIM_DRONE_ID를 확인하세요."
                )
            victim_position = [
            ...
            victim_index = -1
            spawn_description = (
                f"TEST {FOR_TEST_VICTIM_DRONE_ID} "
                "first-row visible clearing"
            )
        else:
            victim_index = int(
