...
- 착륙 시험 시 1번 드론 첫 수색 줄 중간에 조난자를 자동 배치한다.
"""
...
# 0.0은 첫 줄 시작, 0.5는 중간, 1.0은 첫 줄 끝이다.
FOR_TEST_VICTIM_FIRST_ROW_PROGRESS = 0.50
...
class ForestRescueSimulation:
    ...
    def _write_generated_search_plan(self):
        ...
        for config, zone_index in zip(DRONE_CONFIGS, zone_indices):
            ...
            # 1번 드론의 첫 번째 가로 수색 줄 중 지정 비율 지점을
            # 착륙 복귀 시험용 조난자 위치로 저장한다.
            if (
                FOR_TEST_VICTIM_SPAWN_ENABLED
                and drone_name == FOR_TEST_VICTIM_DRONE_ID
            ):
                progress = float(np.clip(
                    FOR_TEST_VICTIM_FIRST_ROW_PROGRESS,
                    0.0,
                    1.0,
                ))
                test_x = x_low + (x_high - x_low) * progress
                test_y = float(rows[0])
                ...
                print(
                    "[TEST] 조난자 시험 위치 계산: "
                    f"drone={drone_name}, first_row={progress:.2f}, "
    ...
    def _spawn_people(self):
        ...
        if FOR_TEST_VICTIM_SPAWN_ENABLED:
            ...
            spawn_description = (
                ...
                f"first-row {FOR_TEST_VICTIM_FIRST_ROW_PROGRESS:.2f}"
            )
        else:
            victim_index = int(
