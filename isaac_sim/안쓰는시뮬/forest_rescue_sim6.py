...
# 시작 발판 배치
# ---------------------------------------------------------------------------
# /World/layout 아래의 Cube가 구조팀 시작 발판이다.
# Cube의 하드코딩된 Translate 값이 아니라 실제 World Bounding Box를 읽어서
# 구조자 1명과 드론 3대를 Cube 윗면 위에 자동 배치한다.
LAYOUT_PRIM_PATH = "/World/layout"
START_PLATFORM_PRIM_PATH = "/World/layout/Cube"

# Cube 중심을 기준으로 구조자와 드론을 배치할 상대 오프셋이다.
# 실제 최종 World 좌표는 환경을 불러온 다음 Cube의 윗면에서 계산된다.
RESCUER_XY_OFFSET = np.array([0.0, -1.0], dtype=float)
DRONE_X_OFFSETS = (-1.8, 0.0, 1.8)
DRONE_ROW_Y_OFFSET = 1.0
RESCUER_GROUND_CLEARANCE_M = 0.03
DRONE_GROUND_CLEARANCE_M = 0.25
...
class ForestRescueSimulation:
    def __init__(self):
        ...
        self._configure_start_platform_spawn()
        ...
        print(
            f"[INFO] Start platform: {self.start_platform_prim_path}"
        )
        print(
            f"[INFO] Rescuer position on Cube: {self.rescuer_position}"
        )
        print(
            f"[INFO] Victim spawn: {self.selected_victim_spawn['name']} "
            f"at {self.selected_victim_spawn['position']}"
        )
        for spec in self.drone_specs:
            name = spec["name"]
            print(
    ...
    def _find_start_platform_prim(self):
        """`/World/layout` 아래에서 시작 발판 Cube Prim을 찾는다."""
        stage = omni.usd.get_context().get_stage()

        # 가장 먼저 사용자가 말한 표준 경로를 확인한다.
        exact_prim = stage.GetPrimAtPath(START_PLATFORM_PRIM_PATH)
        if exact_prim and exact_prim.IsValid():
            return exact_prim

        layout_prim = stage.GetPrimAtPath(LAYOUT_PRIM_PATH)
        if not layout_prim or not layout_prim.IsValid():
            raise RuntimeError(
                "시작 발판을 찾기 위한 layout Prim이 없습니다.\n"
                f"Expected: {LAYOUT_PRIM_PATH}"
            )

        # Cube/cube처럼 대소문자가 다르거나 Cube가 한 단계 더 아래에 있는
        # 경우를 고려해 layout의 모든 하위 Prim을 검색한다.
        exact_name_matches = []
        partial_name_matches = []

        for prim in Usd.PrimRange(layout_prim):
            if prim == layout_prim:
                continue
            ...
            if name == "cube":
                ...
                partial_name_matches.append(prim)

        candidates = exact_name_matches or partial_name_matches
        if not candidates:
            raise RuntimeError(
                "`/World/layout` 아래에서 시작 발판 Cube Prim을 "
                "찾지 못했습니다."
            )
        ...
        if len(candidates) > 1:
            ...
            carb.log_warn(
                "Cube 후보가 여러 개라 첫 번째 Prim을 시작 발판으로 "
                f"사용합니다: {candidate_paths}"
            )

        return sorted(candidates, key=lambda prim: str(prim.GetPath()))[0]

    def _configure_start_platform_spawn(self):
        """Cube의 실제 윗면 좌표로 구조팀 스폰 위치를 계산한다."""
        platform_prim = self._find_start_platform_prim()
        self.start_platform_prim_path = str(platform_prim.GetPath())
        ...
        platform_range = bbox_cache.ComputeWorldBound(
            platform_prim
        ).ComputeAlignedRange()

        if platform_range.IsEmpty():
            raise RuntimeError(
                "시작 발판 Cube의 World Bounding Box를 계산하지 "
                f"못했습니다: {self.start_platform_prim_path}"
            )

        minimum = platform_range.GetMin()
        maximum = platform_range.GetMax()
        center = 0.5 * (minimum + maximum)

        width_x = float(maximum[0] - minimum[0])
        width_y = float(maximum[1] - minimum[1])
        platform_top_z = float(maximum[2])

        # 발판이 예상보다 작아도 가장자리 밖으로 나가지 않도록 기존 배치
        # 간격을 발판 크기에 맞춰 자동으로 줄인다.
        usable_half_x = max(0.0, width_x * 0.5 - 0.6)
        usable_half_y = max(0.0, width_y * 0.5 - 0.6)
        drone_x_spacing = min(1.8, usable_half_x)
        row_y_offset = min(DRONE_ROW_Y_OFFSET, usable_half_y)
        rescuer_y_offset = min(abs(RESCUER_XY_OFFSET[1]), usable_half_y)

        if drone_x_spacing < 0.8 or row_y_offset < 0.4:
            carb.log_warn(
                "시작 발판 Cube가 구조자와 드론 3대를 배치하기에 "
                f"작을 수 있습니다: size=({width_x:.2f}, {width_y:.2f})"
            )

        center_x = float(center[0])
        center_y = float(center[1])

        self.rescuer_position = [
            center_x + float(RESCUER_XY_OFFSET[0]),
            center_y - rescuer_y_offset,
            platform_top_z + RESCUER_GROUND_CLEARANCE_M,
        ]

        drone_x_offsets = (-drone_x_spacing, 0.0, drone_x_spacing)
        self.drone_specs = tuple(
            {
                "name": f"rescue_drone_{index + 1}",
                "vehicle_id": index,
                "position": [
                    center_x + float(x_offset),
                    center_y + row_y_offset,
                    platform_top_z + DRONE_GROUND_CLEARANCE_M,
                ],
            }
            for index, x_offset in enumerate(drone_x_offsets)
        )

        print(
            "[CHECK] Start platform Cube: "
            f"{self.start_platform_prim_path}"
        )
        print(
            "[CHECK] Start platform bounds: "
            f"min={minimum}, max={maximum}, "
            f"size=({width_x:.2f}, {width_y:.2f})"
        )
        print(
            "[CHECK] Start platform top center: "
            f"({center_x:.3f}, {center_y:.3f}, {platform_top_z:.3f})"
        )
    ...
    def _spawn_rescuer(self):
        ...
        self.rescuer = Person(
            "rescuer_01",
            selected_asset,
            init_pos=self.rescuer_position,
            init_yaw=0.0,
        )

        print(f"[INFO] Rescuer asset: {selected_asset}")
        print(f"[INFO] Rescuer spawn on Cube: {self.rescuer_position}")
