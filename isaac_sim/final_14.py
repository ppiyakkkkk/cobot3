...
# 드론 World 위치를 기준으로 카메라를 X/Y 뒤쪽, Z 위쪽에 배치한다.
# World 고정 오프셋이므로 드론이 회전해도 화면 방향이 급격히 돌지 않는다.
FOLLOW_CAMERA_WORLD_OFFSET = np.array(
    [-12.0, -12.0, 7.0],
    dtype=np.float64,
)
FOLLOW_CAMERA_TARGET_HEIGHT_M = 0.8
...
class ForestRescueSimulation:
    ...
    def _setup_follow_viewport(self):
        ...
        try:
            ...
            print(
                "[INFO] Main Viewport follow camera enabled: "
                f"target={FOLLOW_DRONE_PRIM_PATH}, "
                f"camera={FOLLOW_CAMERA_PRIM_PATH}, "
                f"offset={FOLLOW_CAMERA_WORLD_OFFSET.tolist()}"
            )
        except Exception as error:
            self._follow_viewport_api = None
            self._follow_camera_ready = False
    ...
    def _update_follow_viewport(self):
        """드론 현재 World 위치에 맞춰 추적 카메라를 매 프레임 이동한다."""
        ...
        drone_position = world_matrix.ExtractTranslation()
        ...
        eye = target + FOLLOW_CAMERA_WORLD_OFFSET
