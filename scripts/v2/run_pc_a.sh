...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --rviz) USE_RVIZ=true ;;
        -h|--help)
            echo "사용법: run_pc_a.sh [--drone-count 1~4] [--mode MODE] [--rviz]"
            exit 0
            ;;
        *) die "알 수 없는 인자: $1" ;;
    esac
...
echo "[RUN] launch=forest_rescue_pc_a.launch.py, drone_count=${DRONE_COUNT}, mode=${MODE}, rviz=${USE_RVIZ}"

exec ros2 launch forest_rescue_system forest_rescue_pc_a.launch.py \
